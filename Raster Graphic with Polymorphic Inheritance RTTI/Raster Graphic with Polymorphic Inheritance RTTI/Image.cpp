...
ostream& operator<<(ostream& o, Image& image) {
	...
	try {
		GPUMemoryImage& GMI = dynamic_cast<GPUMemoryImage&>(image);
		o << "GPU Memory Image. Shader = " << GMI.GetShader() << endl;
	}
	catch (const bad_cast e) {}

	try {
		SystemMemoryImage& SMI = dynamic_cast<SystemMemoryImage&>(image);
		o << "System Memory Image" << endl;
	} catch (const bad_cast e) {}

	o << "\tImage name = " << image.name << "; ";
	...
	o << "\tMemory requirements = " << image.BufferSize();
	o << endl;
